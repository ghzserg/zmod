...
class MCU_I2C:
    def __init__(self, mcu, bus, addr, speed, sw_pins=None, radar_interval=0):
        self.mcu = mcu
        self.bus = bus
        self.i2c_address = addr
        self.interval=radar_interval
        ...
        if sw_pins is not None:
            self.config_fmt = (
                ...
                " scl_pin=%s sda_pin=%s rate=%d address=%d radar_interval=%d"
                % (self.oid, sw_pins[0], sw_pins[1], speed, addr, radar_interval))
        else:
            self.config_fmt = (
        ...
        self.i2c_set_interval_cmd = None
        self.i2c_set_pulse_cmd = None
    def get_oid(self):
        return self.oid
    def get_mcu(self):
        return self.mcu
    ...
    def build_config(self):
        ...
        self.i2c_modify_bits_cmd = self.mcu.lookup_command(
            ...
            cq=self.cmd_queue)
        self.i2c_set_interval_cmd = self.mcu.lookup_command(
            "set_radar_interval oid=%c radar_interval=%u",cq=self.cmd_queue)
        self.i2c_set_pulse_cmd = self.mcu.lookup_command(
            "set_radar_sync_pulse oid=%c pulse_cnt=%u pulse_interval=%u",cq=self.cmd_queue)
    ...
    def set_interval(self, data, minclock=0, reqclock=0):
        self.i2c_set_interval_cmd.send([self.oid, data],
                minclock=minclock, reqclock=reqclock)
    def set_pulse(self, val, num, minclock=0, reqclock=0):
        print("val:",val)
        print("num:",num)
        self.i2c_set_pulse_cmd.send([self.oid, num, val],
                minclock=minclock, reqclock=reqclock)
...
def MCU_I2C_from_config(config, default_addr=None, default_speed=100000, default_interval=0):
    # Load bus parameters
    ...
    # Determine pin from config
    interval = config.getint('interval', default_interval, minval=0, maxval=8800)
    ppins = config.get_printer().lookup_object("pins")
    ...
    return MCU_I2C(i2c_mcu, bus, addr, speed, sw_pins, interval)
