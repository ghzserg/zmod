...
IMG_HEIGHT_ADDR=5
IMG_WIDTH_ADDR=9
CAM_OFFSET_X_ADDR=13
CAM_OFFSET_Y_AADR=18
TAG_OFFSET_X_ADDR=23
TAG_OFFSET_Y_ADDR=28
INNER_MAT_ADDR=33
DIST_ADDR=53
...
class EEPROMCommandHelper:
    ...
    def register_commands(self, name):
        ...
        gcode.register_mux_command("SET_INTERVAL", "CHIP", name,
                                   self.cmd_SET_INTERVAL)
        gcode.register_mux_command("SET_PULSE", "CHIP", name,
                                    self.cmd_SET_PULSE)
    ...
    def cmd_SET_PULSE(self, gcmd):
        val = gcmd.get_int('VAL')
        num = gcmd.get_int('NUM')
        self.chip.set_pulse(val,num)
    def cmd_SET_INTERVAL(self, gcmd):
        val = gcmd.get_int('VAL')
        self.chip.set_interval(val)
    ...
    def cmd_EEPROM_DEBUG_WRITE_FLOAT(self, gcmd):
        ...
        gcmd.respond_info("EEPROM_DEBUG_WRITE_FLOAT : ADDR[0x%x] = 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x"
                        % (addr, vals[0], vals[1], vals[2], vals[3], vals[4], vals[5], vals[6], vals[7]))
        self.chip.write_reg(addr, vals)
...
class BL24C16F:
    ...
    def set_interval(self,val):
        self.i2c0.set_interval(val)
    def set_pulse(self, val, interval):
        self.i2c0.set_pulse(val,interval)
