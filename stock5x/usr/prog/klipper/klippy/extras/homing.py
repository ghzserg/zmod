...
class PrinterHoming:
    def __init__(self, config):
        ...
        gcode.register_command('ADJUST_ENDSTOP_POS', self.cmd_ADJUST_ENDSTOP_POS)
    ...
    def cmd_G28(self, gcmd):
        ...
        try:
            ...
            raise
    def cmd_ADJUST_ENDSTOP_POS(self, gcmd):
        offset = gcmd.get_float('OFFSET', 0.)
        stepper_name = gcmd.get('STEPPER', None)
        if stepper_name is None:
            return
        configfile = self.printer.lookup_object('configfile')
        configfile.set(stepper_name, 'position_endstop', "%.3f" % (offset,))
        gcode = self.printer.lookup_object('gcode')
        gcode.respond_info(
            "%s: y_offset: %.3f\n"
