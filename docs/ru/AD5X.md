...
- receive_delay - сколько ждать между отправкой команды IFS и чтением ответа (*0.2*)
- next_cmd_delay - сколько ждать между обработкой ответа IFS и отправкой следующей команды (*0.2*)
- send_ff_terminator - отправлять ли завершающий байт 0xFF в IFS после *receive_delay*, может потребоваться для старой прошивки IFS (*True*, False)
