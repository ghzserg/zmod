...
#### **Сброс пластика, управляемый слайсером**

Существует возможность передать управление сбросом (purging) слайсеру, используя другие значения параметра USE_TRASH_ON_PRINT вместо значения по умолчанию (1).

Репозиторий zmod_preprocess содержит профили OrcaSlicer для использования в этих режимах. При использовании этих профилей вы можете переключаться в режим nopoop.

##### Режим Nopoop (`SAVE_ZMOD_DATA USE_TRASH_ON_PRINT=0`)

В этом режиме принтер не выполняет сброс пластика во время смены цвета. Принтер обрезает нить, возвращается к черновой башне (prime tower) для выгрузки и загрузки филамента, после чего сразу продолжает печать.

На первом слое вместо этого принтер переместится к лотку во время смены филамента, но затем вернется к черновой башне, не производя отходов (poop).

Для надлежащей очистки от старого филамента в этом режиме рекомендуется включить опцию «Purge into prime tower» (Сброс в черновую башню) в настройках OrcaSlicer. Она находится в настройках принтера на вкладке «Multimaterial». Затем вы можете использовать параметр «Flush Volumes» для регулировки объема сброса. Если вы хотите добавить фиксированный объем к автоматически рассчитанным значениям пролива, вы можете сделать это, установив «Nozzle Volume» (Объем сопла) на вкладке «General» в настройках принтера.

Это нормально, что ваша черновая башня будет значительно больше обычного при использовании этой опции, особенно при печати слоями небольшой высоты.

Кроме того, в этом режиме можно использовать такие опции, как «Purge to infill» (Сброс в заполнение), «Purge to this object» (Сброс в этот объект) и т.д., чтобы уменьшить количество отходов, сбрасываемых в черновую башню.

##### Режим сброса, управляемый слайсером (`SAVE_ZMOD_DATA USE_TRASH_ON_PRINT=2`)

В этом режиме принтер не выполняет сброс пластика самостоятельно во время смены цвета. Принтер обрезает нить, перемещается к лотку и возвращает управление слайсеру.

Этот режим требует надлежащей поддержки со стороны профиля принтера в слайсере, в частности, необходим G-код смены филамента, который обрабатывает сброс пластика и последующий возврат к черновой башне. НЕ ИСПОЛЬЗУЙТЕ этот режим с любым G-код файлом, который не был специально нарезан (sliced) для него.

Такие опции, как «Purge to infill», не могут быть использованы в этом режиме. Это ошибка в OrcaSlicer, которую zMod не может исправить.

##### Профили принтера

Профили принтера, настроенные для сброса под управлением слайсера, доступны в репозитории zmod_preprocess. Эти профили близки к стандартным профилям AD5X, за исключением:
- Добавлен весь пользовательский G-код zMod, включая соответствующий G-код смены филамента для USE_TRASH_ON_PRINT=2
- Включена опция «Purge in prime tower»
- Автоматически устанавливается правильное значение USE_TRASH_ON_PRINT в начале печати
- Тип Z-Hop установлен в «Normal»
- Объем сопла (Nozzle volume) установлен на 144
- Время выгрузки филамента установлено для более точной оценки времени (на основе настроек по умолчанию в filament.json)

При использовании этих профилей вы можете переключаться между режимом Nopoop (по умолчанию) и режимом сброса под управлением слайсера, просто включая или выключая опцию «Purge in prime tower» в настройках принтера.

**Если вы печатаете из этих профилей в режиме сброса под управлением слайсера, обязательно верните значение параметра USE_TRASH_ON_PRINT на 0 или 1 перед печатью любого многоцветного G-кода, который не был нарезан с использованием этих профилей.**
