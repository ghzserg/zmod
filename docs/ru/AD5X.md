...
Этот режим требует надлежащей поддержки со стороны профиля принтера в слайсере; в частности, необходим G-код смены филамента, который обрабатывает сброс пластика и последующий возврат к черновой башне. НЕ используйте этот режим с любым G-код файлом, который не был специально нарезан (sliced) под него.

При использовании OrcaSlicer в этом режиме нельзя использовать такие опции, как «Очистка в заполнение». Это ошибка в OrcaSlicer, которую невозможно исправить через zMod. В Bambu Studio они работают корректно.

##### Профили принтеров

Профили принтеров, настроенные для очистки под управлением слайсера, доступны для [OrcaSlicer](https://github.com/ghzserg/zmod_preprocess/tree/main/profiles/orcaslicer) и [Bambu Studio](https://github.com/ghzserg/zmod_preprocess/tree/main/profiles/bambustudio). Эти профили близки к стандартным профилям AD5X, за исключением:
