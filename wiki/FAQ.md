...
| [MCU Protocol error](#mcu-protocol-error) | [Кончился или остановился филамент](https://wiki.zmod.link/ru/FAQ/#кончился-или-остановился-филамент) |
...
- Поддержка [AD5X](https://wiki.zmod.link/ru/AD5X/)
- Поддержка [следующих языков](https://wiki.zmod.link/ru/Global/#lang): Английский, Немецкий, Французский, Итальянский, Испанский, Китайский, Японский, Корейский 
- Работа с родным экраном
- [Восстановление печати при сбое питания](https://wiki.zmod.link/ru/Zmod/#zrestore)
- [Снятие шейперов c генерацией графиков](https://wiki.zmod.link/ru/Calibrations/#zshaper) с учетом [SCV](https://wiki.zmod.link/ru/Global/#fix_scv) ([square_corner_velocity](https://www.klipper3d.org/Config_Reference.html#printer)).
- [Проверка и восстановление файлов/прав/символических ссылок родной файловой системы](https://wiki.zmod.link/ru/System/#check_system)
- Автоматическое обновление `Fluidd`/`Mainsail`/`Moonraker` и ZMOD по сети
- [Entware](https://wiki.zmod.link/ru/FAQ/#в-zmod-входит-entware---как-им-воспользоваться)
- Исправлена ошибка [E0017](https://wiki.zmod.link/ru/System/#fix_e0017)
...
- Адаптивное снятие карты стола [KAMP](https://wiki.zmod.link/ru/Calibrations/#kamp)
- Калибровка PID [экструдера](https://wiki.zmod.link/ru/Calibrations/#pid_tune_extruder) и [стола](https://wiki.zmod.link/ru/Calibrations/#pid_tune_bed) в том числе через GuppyScreen.
- Реализован [COLDPULL/Колдпул](https://wiki.zmod.link/ru/Filament/#coldpull) (очистка сопла) без насилия. Реализация [этого алгоритма](https://t.me/FF_5M_5M_Pro/2836/447172)
...
- Глобальные настройки мода хранятся через макрос [SAVE_ZMOD_DATA](https://wiki.zmod.link/ru/Global/#save_zmod_data) *нюхлер*
...
- Если перезапускается klipper (после сохранения карты стола, PID, Шейперов и т.д.), то родной экран замерзает (используйте перезапуск через [NEW_SAVE_CONFIG](https://wiki.zmod.link/ru/Main/#new_save_config)
- После отмены печати, на родном экране надо нажать кнопку Ок (используйте макрос [CLOSE_DIALOGS](https://wiki.zmod.link/ru/Main/#close_dialogs) или [FAST_CLOSE_DIALOGS](https://github.com/ghzserg
...
- Необходимо удалить весь стартовый gcode и прописать [START_PRINT](https://wiki.zmod.link/ru/Main/#start_print), а в конечный [END_PRINT](https://wiki.zmod.link/ru/Main/#end_print)
- Не работает камера, необходимо запустить альтернативную камеру через макрос [CAMERA_ON](https://wiki.zmod.link/ru/Zmod/#camera_on)
- При необходимости, надо вручную прописать параметр `Z_OFFSET` в макросе [START_PRINT](https://wiki.zmod.link/ru/Main/#start_print) или использовать глобальный параметр [LOAD_ZOFFSET](https://wiki.zmod.link/ru/Global/#load_zoffset) который загружает Z-offset из глобальных параметров, сохраненных ранее через SET_GCODE_OFFSET. *крот*
...
Если [отключить родной экран](https://wiki.zmod.link/ru/System/#display_off), то мы экономим память.
...
Также, при работе в режиме без родного экрана, принтер не выставляет Z-offset записанный на экране, и его необходимо передавать как параметр в [START_PRINT](https://wiki.zmod.link/ru/Main/#start_print) или через глобальные параметры. [Подробнее](https://wiki.zmod.link/ru/FAQ/#как-работает-z-offset)
...
- [Установите последнее обновление родной прошивки и ZMOD](https://wiki.zmod.link/ru/Recomendations/#установите-последнее-обновление-родной-прошивки-и-zmod)
- Прочитайте [особенности работы](#известные-особенности)
- Вы могли отключить экран. Включите его макросом [DISPLAY_ON](https://wiki.zmod.link/ru/System/#display_on)
...
Прочитайте документацию к [START_PRINT](https://wiki.zmod.link/ru/Main/#start_print) и [SAVE_ZMOD_DATA](https://wiki.zmod.link/ru/Global/#save_zmod_data), это позволит вам использовать расширенные и полезные функции ZMOD

Если хотите воспользоваться откатом из прошивки, то прочитайте [документацию](https://wiki.zmod.link/ru/FAQ/#что-такое-откат-из-прошивки) и добавьте 
...
Прочитайте статью "[Как работает Z-Offset на нашем принтере](https://wiki.zmod.link/ru/Setup/#как-работает-z-offset-на-вашем-принтере)"
...
Любой вызов `SET_GCODE_OFFSET` (а он автоматически вызывается при изменении смещения Z из Fluid/Mainsail/GuppyScreen) сохраняет текущий z-offset в глобальные параметры мода. Но это сохраненное значение используется только в том случае, если указан глобальный параметр [LOAD_ZOFFSET](https://wiki.zmod.link/ru/Global/#load_zoffset)(который по умолчанию отключен, для вклчючения `SAVE_ZMOD_DATA LOAD_ZOFFSET=1`), не используется родной экран и используется макрос [START_PRINT](https://wiki.zmod.link/ru/Main/#start_print).

Также для задания Z-offset можно использовать параметры [START_PRINT](https://wiki.zmod.link/ru/Main/#start_print)
...
- Снять заранее карту стола через макрос [AUTO_FULL_BED_LEVEL](https://wiki.zmod.link/ru/FAQ/#через-макросы-и-кнопки-во-fluidd). 
 
  ```AUTO_FULL_BED_LEVEL EXTRUDER_TEMP=230 BED_TEMP=80 PROFILE=moya_karta_na_80_gradusov```

Выбрать один из двух вариантов:
- Указать название карты стола в параметре `MESH` для макроса [START_PRINT](https://wiki.zmod.link/ru/Main/#start_print). 
...
- Если работаете в режиме без родного экрана и используете макрос [START_PRINT](https://wiki.zmod.link/ru/Main/#start_print) в начальном G-коде, то при каждой печати тоже будет сниматься карта стола

  Для включения этой возможности нужно настроить один раз макрос [SAVE_ZMOD_DATA](https://wiki.zmod.link/ru/Global/#save_zmod_data), параметр [PRINT_LEVELING](https://wiki.zmod.link/ru/Global/#zshaper)
...
- Можно включить адаптивное снятие карты стола (KAMP), тогда  будет сниматься не весь стол, а только те части, где есть модели для печати. 
  **Автоматически снятие карты стола не запускается!**. Этот параметр говорит, что если вызывается снятие карты стола, то запускать вместо него KAMP.
  ...
  Для включения этой возможности нужно настроить один раз макрос [SAVE_ZMOD_DATA](https://wiki.zmod.link/ru/Global/#save_zmod_data), параметр [USE_KAMP](https://wiki.zmod.link/ru/Global/#zshaper)
...
Если вы не хотите пользоваться глобальными параметрами *(SAVE_ZMOD_DATA PRINT_LEVELING=0)*, то вам доступны следующие параметры макроса [START_PRINT](https://wiki.zmod.link/ru/Main/#start_print), который прописывается в стартовый G-код.
...
Алгоритм снятия карты стола в макросе [START_PRINT](https://wiki.zmod.link/ru/Main/#start_print):
...
- [AUTO_FULL_BED_LEVEL](https://wiki.zmod.link/ru/Calibrations/#auto_full_bed_level) - снятие карты стола с очисткой сопла при заданной температуре стола и экструдера. *Выключает нагрев после снятия карты.*. 
   ...
   M104 S[nozzle_temperature_initial_layer]
   ```
- [KAMP](https://wiki.zmod.link/ru/Calibrations/#kamp) - Адаптивная калибровка стола с очисткой сопла
...
- [CLEAR_NOZZLE](https://wiki.zmod.link/ru/Global/#clear_nozzle) - Очистка сопла как в родной прошивке
- Параметр [PRECLEAR](https://wiki.zmod.link/ru/Global/#preclear) - Дополнительная очистка сопла при снятии карты стола.
- Параметр [CLEAR](https://wiki.zmod.link/ru/Global/#clear) - четыре алгоритма (можно добавить свой) очистки сопла линией, перед печатью.
...
- [Частые вопросы](https://wiki.zmod.link/ru/FAQ/)
- [Рекомендации по повышению стабильности работы принтера](https://wiki.zmod.link/ru/Recomendations/)
...
Альтернативная камера, позволяет изменить разрешение, fps, разрешить множественные подключения, не пережимает картинку, легко перезапускается и настраивается [макросом](https://wiki.zmod.link/ru/Zmod/#camera_on). *заяц*

- Отключите на экране принтера родную камеру.
- Вызовите макрос [CAMERA_ON](https://wiki.zmod.link/ru/Zmod/#camera_on)
...
Если хотите настраивать разрешение, fps, использовать камеру из Telegram бота, снизить потребление оперативной памяти, разрешить параллельные подключения, то нужно использовать [альтернативную камеру](https://wiki.zmod.link/ru/Zmod/#camera_on). *крот*
...
[Как настроить в Orca](https://wiki.zmod.link/ru/Recomendations/#отправляйте-файлы-на--печать-через-octoklipper)

### Я переключил веб интерфейс и теперь ничего не работает.

Если вы переключили интерфейс макросом [WEB](https://wiki.zmod.link/ru/System/#web) *выхухоль*
...
**Внимание! В [AD5X](https://wiki.zmod.link/ru/AD5X/) нет Entware**
...
[Инструкция перенесена](https://wiki.zmod.link/ru/Help/)
...
- Можно прекращать печать, если сопло ударилось о деталь или детать была оторвана. [NOZZLE_CONTROL](https://wiki.zmod.link/ru/Global/#nozzle_control)
...
- [Устанавливаем родную Factory прошивку](https://wiki.zmod.link/ru/Native_FW/#как-установить-родную-прошивку), которая установит родной MCU.
...
- В экструдере пруток 1, а вытягивается пруток 2. Используйте [SET_EXTRUDER_SLOT](https://wiki.zmod.link/ru/AD5X/#5-как-вручную-указать-принтеру-какая-катушка-сейчас-заправлена-ad5x).
- В экструдер вставляется пруток, но в нем уже есть старый пруток
- Модули 4 в 1 и трубки к ним имеют разную длину, поэтому нужно отрегулировать параметр ```nozzle_cleaning_length``` в ```mod_data/filament.json``` установив его 70 или больше. [Подробнее](https://wiki.zmod.link/ru/AD5X/#самые-важные-настройки-что-менять-чаще-всего-ad5x)
...
После этого протестировать печать и блокировку/разблокировку прутков через [команды IFS](https://wiki.zmod.link/ru/AD5X/#10-ifs-команды).
...
Решение:
- [Откатить родную прошивку](https://wiki.zmod.link/ru/Native_FW/) на версию **1.1.7** для AD5X, **3.2.3** для FF5M/FF5MPro
- [Отключить родной экран](https://wiki.zmod.link/ru/System/#display_off)
...
Если не получается исправить, то [нужно создать тикет](https://wiki.zmod.link/ru/Help/).
