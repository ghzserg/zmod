...
- 3 - тестировать с АВТОПОДБОРОМ Z-Offset, без очистки сопла
- 4 - тестировать с АВТОПОДБОРОМ Z-Offset, без очистки сопла, в случае несовпадения карты, запускать KAMP
- 5 - тестировать с АВТОПОДБОРОМ Z-Offset, с очисткой сопла
- 6 - тестировать с АВТОПОДБОРОМ Z-Offset, с очисткой сопла, в случае несовпадения карты, запускать KAMP
...
2.  **Подготовка (опционально):**
    *   Если активирована очистка сопла, оно прогревается до рабочей температуры, очищается о стол и остывает до 140°C.
3.  **Выбор точки измерения:**
    *   Если сопло было очищено — используется **центральная** точка карты.
    *   Если нет — выбирается **свободная** точка, не занятая моделью для печати.
...
5.  **Корректировка смещения:**
    *   Если расхождение **меньше 0.6 мм**, разница добавляется к текущему значению Z-Offset.
    *   Если расхождение **больше или равно 0.6 мм**, система считает сохраненную карту неактуальной и, при разрешенных настройках, автоматически запускает процедуру повторного выравнивания стола (KAMP).
...
Если используется датчик наличия филамента без родного экрана, то при сработке Выводится фраза: "Кончился филамент. Через 30 секунд будет пауза"
...
Только при работе без родного экрана
